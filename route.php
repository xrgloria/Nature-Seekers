...
session_start();
if(!isset($_SESSION['user_id']) and !isset($_GET['routeID'])){
		...
		<div class="panel panel-primary route">
			...
			<div class="panel-body routeContent" id="overlayList">
				...
				if(isset($_GET['routeID']))
				{
					...
					$selectQuery = "SELECT O.OVERLAY_ID, O.OVERLAY_NAME,O.ACTIVITY_NAME, P.LATITUDE, P.LONGITUDE
									FROM OVERLAYS O
					...
					if (mysql_num_rows($result) > 0) {
						while($row = mysql_fetch_array($result)) {
							...
							echo '<div class="route-A" id="'. $overlayID . '">';
							echo '<b>' . $row['OVERLAY_NAME'] . '</b><br />';
							echo $row['ACTIVITY_NAME'];
							echo '<br />';
							$divClass = !$divClass;
							echo '<input type="hidden" name="overlayID[]" value="' . $overlayID . '"/>';
							echo '</div>';
							array_push($overlayPoints, array( $row['LATITUDE'],$row['LONGITUDE']));
						}
...
<?php
if(isset($_GET['routeID'])){
		echo '<script >function addAllMarkers(){';
